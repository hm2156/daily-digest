...
const Home = () => {
  ...
  // New state to track broken images
  ...
  useEffect(() => {
    ...
    // Clear broken images state when fetching a new set of articles
    setBrokenImages({});
    ...
    const fetchNews = async () => {
      ...
      try {
        ...
        const endpoint = isSearchMode
          ? `https://newsapi.org/v2/everything?q=${searchQuery}&language=en&apiKey=${newsApiKey}`
  ...
  const handleToggleTrendChart = () => {
    ...
    // NEW FUNCTION: Handle click on the title
    const handleTitleClick = () => {
      ...
      // NEW FUNCTION: Handle going back from About page
      const handleBackToHome = () => {
        setShowAboutPage(false);
        setCategory("general"); // Reset to show top headlines
      };
  ...
  return (
    ...
    <header className="py-8 border-b border-gray-200 mb-12">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 
        ...
        <div>
          {/* MODIFIED: Wrapped the h1 in a button to make it clickable */}
          <button
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className={`px-5 py-2 rounded-full text-sm font-medium transition-colors duration-200 flex items-center justify-center gap-2 min-w-[120px]
              ${showTrendChart
          ...
          <button
            onClick={() => {
              ...
              if (!showSavedArticles) {
                ...
                setCategory("general");
                setSearchQuery("");
                // MODIFIED: Also hide the About page
                setShowAboutPage(false);
              }
            }}
            className={`px-5 py-2 rounded-full text-sm font-medium transition-colors duration-200 flex items-center justify-center gap-2 min-w-[120px]
    ...
    {/* NEW CONDITIONAL RENDERING for the About page */}
    {showAboutPage ? (
      ...
      <>
        <div className="bg-white rounded-xl p-8 shadow-lg mb-8 transition-shadow duration-300">
          ...
          <p className="text-sm text-blue-700 mb-6">
              <strong className="font-bold">Note:</strong> Due to NewsAPI limitations and usage caps, we recommend using this feature sparingly.
          </p>
          <form
              onSubmit={handleGenerateTrend}
          ...
          >
            <input
              ...
              className="flex-1 w-full sm:w-auto bg-gray-100 rounded-full px-5 py-3 text-gray-800 
              placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all 
              duration-200 text-lg border-2 border-transparent focus:border-blue-500"
            />
            <select
              value={trendDays}
              onChange={(e) => setTrendDays(Number(e.target.value))}
              className="w-full sm:w-auto bg-gray-100 rounded-full px-5 py-3 text-gray-800 
              focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all 
              duration-200 text-lg appearance-none cursor-pointer border-2 border-transparent focus:border-blue-500"
              style={{
            ...
            <button
              type="submit"
              className="w-full sm:w-auto px-8 py-3 rounded-full bg-gray-800 text-white text-lg 
              font-medium hover:bg-gray-700 transition-colors duration-200 shadow-md hover:shadow-lg"
            >
              Show Trend
            </button>
          </form>
      ...
      <>
        {/* Search Form and Category Buttons - only show if not showing saved articles */}
        {!showSavedArticles && (
          <>
            <form
              onSubmit={(e) => {
        ...
        {/* Conditional Rendering for News List or Saved Articles */}
        {showSavedArticles ? (
                ...
                return (
                  <a href={article.url} target="_blank" rel="noopener noreferrer" key={i} className="block group cursor-pointer">
                    <article className="flex flex-col md:flex-row items-start md:space-x-8">
                      ...
                      {/* Conditional rendering for image or placeholder based on isImageBroken flag */}
              ...
              articles.map((article, i) => {
                ...
                return (
                  <a href={article.url} target="_blank" rel="noopener noreferrer" key={i} className="block group cursor-pointer">
                    <article className="flex flex-col md:flex-row items-start md:space-x-8">
                      <div className="flex-1 order-2 md:order-1">
                        <h2 className="text-3xl font-serif font-extrabold text-gray-900 leading-tight mb-2 group-hover:text-blue-700 transition-colors duration-200">{article.title}</h2>
                        {article.description && (<p className="text-gray-700 text-lg leading-relaxed mb-4">{article.description}</p>)}
                        <p className="text-gray-700 text-base mb-4"><span className="font-semibold">Sentiment:</span> {getMood(article.title)}</p>
